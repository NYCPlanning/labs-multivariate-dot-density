...
generate_dots <- function(file_path, columns = c(), per_dot = 100) {
  ...
  columns <- columns %>% append('temp_buffer')
  ...
  dots <- dots %>%
          ...
          flatten1(
            lapply(
              ...
              function(current, row) {
                return(rep(current, abs(as.integer(row[[current]]/10))))
              },
              .data
            )
          )
    ...
    filter(length(category) > 0) %>%
    mutate(
      category = as.character(sample(category, 1))
    ) %>%
    ungroup
